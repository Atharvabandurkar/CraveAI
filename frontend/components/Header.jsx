...
const Header = async () => {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 border-b border-border bg-background/75 backdrop-blur-md">
      <nav className="container mx-auto h-16 px-4 flex items-center justify-between">
        {/* Logo Section */}
        <Link
          href={user ? "/dashboard" : "/"}
          ...
          <SignedIn>
            {user && (
              <PricingModal subscriptionTier={user.subscriptionTier}>
                <Badge
                  variant="outline"
                  className={`hidden sm:flex h-8 px-3 gap-1.5 rounded-full text-xs font-semibold transition-all cursor-pointer ${
                    user.subscriptionTier === "pro"
                      ? "bg-linear-to-r from-emerald-600 to-cyan-500 text-white border-none shadow-md shadow-emerald-500/20 hover:shadow-emerald-500/40 hover:-translate-y-0.5"
                      : "bg-zinc-200/50 text-zinc-600 border-zinc-200 hover:bg-zinc-200 hover:border-zinc-300"
                  }`}
                >
                  <Sparkles
